...
watch(
  ...
  (newAntidetect) => {
    if (newAntidetect) {
      form.name = newAntidetect.name;
      form.engine = newAntidetect.engine;
      form.deviceType = newAntidetect.deviceType;
      form.osName = newAntidetect.osName;
      form.osVersion = newAntidetect.osVersion;
      form.browserName = newAntidetect.browserName;
      form.browserVersion = newAntidetect.browserVersion;
      form.userAgent = newAntidetect.userAgent;
      form.language = newAntidetect.language;
      form.timezone = newAntidetect.timezone;
      form.locale = newAntidetect.locale;
      form.proxyId = newAntidetect.proxyId;
      form.screenHeight = newAntidetect.screenHeight || 0;
      form.screenWidth = newAntidetect.screenWidth || 0;
      form.deviceScaleFactor = newAntidetect.deviceScaleFactor;
      form.isMobile = newAntidetect.isMobile;
      form.hasTouch = newAntidetect.hasTouch;
      form.canvasMode = newAntidetect.canvasMode;
      form.canvasSeed = newAntidetect.canvasSeed;
      form.webglVendor = newAntidetect.webglVendor || "";
      form.webglRenderer = newAntidetect.webglRenderer || "";
      form.hardwareConcurrency = newAntidetect.hardwareConcurrency;
      form.deviceMemory = newAntidetect.deviceMemory;
    }
  }
);
...
const submit = () => {
  form
    .transform(() => toPayload())
    .patch(`/antidetects/${props.antidetect?.id}/update`, {
      preserveScroll: true,
      preserveState: false,
      onSuccess: () => {
        form.reset();
...
<template>
  <Modal
    v-model:open="isOpen"
    title="Update Antidetect"
    description="Update metadata antidetect."
    @close="emits('close')"
  >
    <template #content>
