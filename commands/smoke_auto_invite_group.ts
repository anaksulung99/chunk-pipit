...
async function waitForCampaignResult(campaignId: string) {
  ...
  throw new Error('Timeout menunggu campaign auto_invite selesai.')
}

export default class SmokeAutoInviteGroup extends BaseCommand {
  static commandName = 'smoke:auto-invite-group'
  static description = 'Smoke nyata auto_invite group memakai worker dan friend pool'
  ...
  async run() {
    ...
    if (!candidateProfiles.length) {
      this.logger.error('Tidak ada candidate profile friend untuk smoke auto invite.')
      return
    }
    ...
    const campaign = await Campaign.create({
      ...
      name: `SMOKE Auto Invite Group ${stamp}`,
      type: 'auto_invite',
      status: 'running',
      config: {
        inviteType: 'group',
    ...
    try {
      ...
      if (inviteLog.status !== 'success') {
        this.logger.error(`Auto invite selesai tetapi log utama bukan success: ${inviteLog.status}`)
        return
      }

      this.logger.success(`Smoke auto_invite group berhasil: ${inviteLog.message}`)
    } finally {
      ...
      this.logger.info('Cleanup smoke auto_invite selesai.')
    }
  }
}
