...
export default class SmokeAutoInvitePageFollowerDirect extends BaseCommand {
  static commandName = 'smoke:auto-invite-page-follower-direct'
  static description = 'Smoke langsung auto_invite page_follower tanpa queue worker untuk isolasi runtime'
  ...
  async run() {
    ...
    if (!candidateProfiles.length) {
      this.logger.error('Tidak ada candidate profile friend untuk smoke auto invite page follower direct.')
      return
    }
    ...
    const campaign = await Campaign.create({
      ...
      name: `SMOKE Auto Invite Page Follower Direct ${stamp}`,
      type: 'auto_invite',
      status: 'running',
      config: {
    ...
    try {
      ...
      if (!inviteLog) {
        this.logger.error('Log auto_invite tidak ditemukan pada direct smoke page follower.')
        return
      }

      if (inviteLog.status !== 'success') {
        this.logger.error(
          `Auto invite page follower direct selesai tetapi log utama bukan success: ${inviteLog.status}`
        )
        return
      }

      this.logger.success(`Smoke auto_invite page follower direct berhasil: ${inviteLog.message}`)
    } finally {
      ...
      this.logger.info('Cleanup smoke auto_invite page follower direct selesai.')
    }
  }
}
