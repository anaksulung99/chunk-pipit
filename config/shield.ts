...
const shieldConfig = defineConfig({
  ...
  csrf: {
     ...
     * The /api/* endpoints are consumed by the Electron client (incl. the
     * main process during the launch gate), which cannot carry a browser
     * CSRF token. Safe here: the API only listens on localhost. Uses the
     * callback form because the string-pattern matcher's `*` only spans a
     * single path segment.
     */
